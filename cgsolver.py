...
class CGSolver:
    ...
    def __init__(self, coef, b, x):
        ...
        # 声明一个尺寸为(self.NX, self.NY)的二维 field, AoS 布局 p[0], Ap[0], r[0], Ax[0] 内存连续
        ti.root.dense(ti.ij, (self.NX, self.NY)).place(
            self.p, self.Ap, self.r, self.Ax)  # Dense data structure
    ...
    def compute_Ap(self):
        for i, j in ti.ndrange((self.N_ext, self.NX-self.N_ext), (self.N_ext, self.NY-self.N_ext)):
            self.Ap[i, j] = self.coef[i, j, 0] * self.p[i, j] + self.coef[i, j, 1] * self.p[i-1, j] + self.coef[i, j, 2] * self.p[i+1, j] +\
                self.coef[i, j, 3] * self.p[i, j+1] + \
                self.coef[i, j, 4] * self.p[i, j-1]

    @ti.kernel
    def compute_Ax(self):
        for i, j in ti.ndrange((self.N_ext, self.NX-self.N_ext), (self.N_ext, self.NY-self.N_ext)):
            self.Ax[i, j] = self.coef[i, j, 0] * self.x[i, j] + self.coef[i, j, 1] * self.x[i-1, j] + self.coef[i, j, 2] * self.x[i+1, j] +\
                self.coef[i, j, 3] * self.x[i, j+1] + \
                self.coef[i, j, 4] * self.x[i, j-1]
    ...
    def check_solution(self):   # Return the norm of rTr as the residual
        ...
        # TODO: Is this equal to compute_residual()?
    ...
    def build_b(self):
        ...
        bsp = bnp[self.N_ext:self.N_tot-self.N_ext,
                  self.N_ext:self.N_tot-self.N_ext]  # Slicing
        b = bsp.flatten(order='C')  # Flatten the array to a 1D vector
        # For debugging purpose
        np.savetxt('b.csv', b, delimiter=',', fmt='%80.70f')
        return b

    def build_x(self):
        xnp = self.x.to_numpy()  # Convert to numpy ndarray
        xsp = xnp[self.N_ext:self.N_tot-self.N_ext,
                  self.N_ext:self.N_tot-self.N_ext]  # Slicing
